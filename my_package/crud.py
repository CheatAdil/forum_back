...
def create_user(db: Session, user: schemas.UserCreate): #
    db_user = models.User(user_email=user.user_email, user_password=get_hash(user.user_password), user_name=user.user_name)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
def update_user(db: Session, user: schemas.UserUpdate): #
    ...
    db_user.user_password = get_hash(user.user_password)
