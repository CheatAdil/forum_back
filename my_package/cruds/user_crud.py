...
from my_package.password_handler import password_handler
...
def create_user(db: Session, user: user_schemas.UserCreate): #
    ph = password_handler
    db_user = models.User(user_email=user.user_email, user_password=ph.get_password_hash(user.user_password), user_name=user.user_name)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
def update_user(db: Session, user: user_schemas.UserUpdate): #
    ...
    ph = password_handler
    db_user.user_password = ph.get_password_hash(user.user_password)
